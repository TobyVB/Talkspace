...
// ##########################################################################
    function changeHeight(loc, value){
        const arr = postObj.inputs
        arr.splice(loc, 1, {...postObj.inputs[loc], height: value+"vh", initializing: false})
        setPostObj(prev => {
            return {...prev, inputs: arr}
        })
    }
...
// ##########################################################################
    ...
    function selectSorter(type){
        ...
        setTextarea(false)
        if(type === "height"){setHeightSettings(true)}
    ...
    function SelectSetting(){
        return(
            <div className="select-setting-btns">
                ...
                <button className={'edit-post-btn'} disabled={heightSettings&&"+true"}  onClick={()=>selectSorter("height")}>height</button>
            </div>
        )
    }
    ...
    const inputs = (inputs) => {
        return inputs.map((input, index) => 
            ...
            {/* {index === 0 && <p className="insert-section-btn" onClick={()=>inserting(-2)}></p>} */}
                ...
                input.type === "text"
                ?
                <p onClick={()=> modifyInput(index)} className={`post-text ${input.output===""&&"empty-field"}`} style={
                ...
                input.type === "video" 
                ?
                input.output===""?<p className="empty-field" onClick={()=> modifyInput(index)}>add video</p>:
                input.output === ""?"field empty":input.output && <iframe className="post-video"  onClick={()=> modifyInput(index)} src={`https://www.youtube.com/embed/${input.output.slice(17)}`} frameBorder="0" allowFullScreen ></iframe>
                :
                input.type === "image" 
                &&
                ...
                {/* #########################   s h o w I n s e r t B t n s  === i n d e x   ######################## */}
                    ...
                    <div className="modifier-section">
                        ...
                        {!insertingMode && showInsertBtns===index&& <Insert key={index+"a"} index={index} />}
                        {!insertingMode && <Shift key={index+"b"} index={index} />}
                        {!insertingMode && <InputOptions key={index+"c"} index={index} />}
                            {/* clicking top arrow gives a state variable a value of -1, clicking anyother arrow sets that state variable to a value of 0 */}
                            {!insertPrevZero && insertingMode && showInsertBtns === index && <InsertBtns index={index+1} />}
                        ...
                        // #################   H E I G H T   ###################
                        heightSettings?
                        <div className="height"><span>height</span>
                            <input name="heightSlider" type="range" min="1" max="100" step="1" value={input.height.slice(0, -2)} 
                            onChange={event => changeHeight(index, event.target.value)}></input>
                        </div>
                        // #####################################################
                        // ###################   F O N T   #####################
                        :fontSettings?
                        ...
                        <input name="fontSize" type="range" min=".1" max="10" step=".1" value={input.fontSize.slice(0, -3)} 
